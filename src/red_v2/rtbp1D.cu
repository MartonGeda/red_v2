...
void rtbp1D::print_solution(std::string& path_si, std::string& path_sd, data_rep_t repres)
{
	...
	case DATA_REPRESENTATION_ASCII:
		...
		if (sout)
		{
			file::rtbp1D::print_solution_info_ascii(sout, t, dt);
		}
		else
		{
	...
	case DATA_REPRESENTATION_BINARY:
		...
		if (sout)
		{
			file::rtbp1D::print_solution_info_binary(sout, t, dt);
		}
		else
		{
			throw string("Cannot open " + path_si + ".");
	...
	case DATA_REPRESENTATION_ASCII:
		...
		if (sout)
		{
			file::rtbp1D::print_solution_data_ascii(sout, n_obj, n_ppo, n_vpo, h_md, h_p, h_y);
		}
		else
		{
			throw string("Cannot open " + path_si + ".");
	...
	case DATA_REPRESENTATION_BINARY:
		...
		if (sout)
		{
			file::rtbp1D::print_solution_data_binary(sout, n_obj, n_ppo, n_vpo, h_md, h_p, h_y);
		}
		else
		{
